...
<head>
    ...
    <title>Editar Participación</title>
</head>
<body>
<div class='container'>
    <h1>Editar Participación</h1>
    <form method="POST" action="{{ route('participacion.update', $participacion->id) }}">
        @method('PUT')
        @csrf
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="id" aria-describedby="idHelp" name="id"
                   disabled="disabled" value="{{ $participacion->id }}">
            <div id="idHelp" class="form-text">Id Participación</div>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="rol" aria-describedby="rolHelp" name="rol"
                   placeholder="Rol Participante" value="{{ $participacion->rol }}">
        </div>

        <div class="mt-3">
            <button type="submit" class="btn btn-primary">Actualizar</button>
            <a href="{{ route('participacion.index') }}" class="btn btn-warning">Cancelar</a>
        </div>
    </form>
</div>
